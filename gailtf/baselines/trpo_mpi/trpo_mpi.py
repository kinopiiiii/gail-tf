...
def learn(env, policy_func, *,
    ...
    # ----------------------------------------
    ob_space = env.observation_space  # 状態空間
    ac_space = env.action_space  # 行動空間
    pi = policy_func("pi", ob_space, ac_space)
    oldpi = policy_func("oldpi", ob_space, ac_space)
    atarg = tf.placeholder(dtype=tf.float32, shape=[None])  # Target advantage function (if applicable) アドバンテージ関数
    ret = tf.placeholder(dtype=tf.float32, shape=[None])  # Empirical return 経験的収益
